...
def setSampleValue(value, sampleIndex):
    timestamp = datetime.now(timezone.utc)
    ...
    setVar(timestamp, P01_SampleIndex, sampleIndex, "integer")
    setVar(timestamp, P02_SampleIndex, sampleIndex, "integer")
    setVar(timestamp, P04_SampleIndex, sampleIndex, "integer")
    setVar(timestamp, P05_SampleIndex, sampleIndex, "integer")
...
P02_SampleIndex = creVar(sim_obj, "P02.SampleIndex", 1, False)
P04_SampleIndex = creVar(sim_obj, "P04.SampleIndex", 1, False)
P05_SampleIndex = creVar(sim_obj, "P05.SampleIndex", 1, False)
...
try:
    ...
    while True:
        ...
        #read every 1s
        timestamp = datetime.now(timezone.utc)
        Heartbeat = Station_Heartbeat.get_value()
        setVar(timestamp, Station_Heartbeat_Ack, Heartbeat, "integer");
        ...
        if State == C_ACQUISITION_STARTED:
            if time.perf_counter() - last_time >= 1.0:
                ...
                if counter == 13:
                    ...
                    setSampleValue(usedValue[index], sampleIndex)
                    index = index + 1
